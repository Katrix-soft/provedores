...
class _DashboardHomeViewState extends State<DashboardHomeView> {
  ...
  Future<void> _loadDashboardData() async {
    ...
    try {
      final metricsData = await apiService.get('/metricas/erp');
      ...
      setState(() {
        ...
        _renewals = alertsList.where((a) => a['tipo_alerta'] == 'vencimiento').toList();
        
        // Contar impagos
        _polizasConDeuda = alertsList.where((a) => a['tipo_alerta'] == 'impago').length;
        
        _isLoading = false;
      });
    } catch (e) {
      print('Error al cargar datos del dashboard: $e');
      // Mantener fallbacks si falla (modo offline)
      setState(() {
        _metrics = {
          'premio_total': 13500000.0,
          'clientes_totales': 8,
          'ramos_distribucion': [
            {'ramo': 'Automotor', 'cantidad': 55},
            {'ramo': 'Hogar', 'cantidad': 25},
            {'ramo': 'Vida', 'cantidad': 15},
            {'ramo': 'Otros', 'cantidad': 5},
          ],
          'companias_distribucion': [
            {'compania': 'Allianz', 'cantidad': 42},
            {'compania': 'Mapfre', 'cantidad': 28},
            {'compania': 'Zurich', 'cantidad': 15},
          ],
        };
        _renewals = [
          {'dias_restantes': 12, 'nro_poliza': '#88219', 'cliente_nombre': 'Juan Pérez', 'urgencia': 1},
          {'dias_restantes': 15, 'nro_poliza': '#99312', 'cliente_nombre': 'Maria Gomez', 'urgencia': 2},
        ];
        _polizasConDeuda = 12;
        _isLoading = false;
      });
            ...
            GridView.count(
              ...
              children: [
                ...
                _buildMetricCard(
                  ...
                  customSubtitle: Row(
                    children: [
                      const Icon(Icons.payments, color: Color(0xFFBA1A1A), size: 14),
                      const SizedBox(width: 4),
                      Text(
                        _polizasConDeuda > 0 ? 'EN DEUDA' : 'AL DÍA',
  ...
  Widget _buildRamosDistribution(BuildContext context, List<dynamic> ramos) {
    final theme = Theme.of(context);
    
    // Calcular cantidad total para los porcentajes
    final double totalCantidad = ramos.fold<double>(
      0.0,
      (sum, item) => sum + ((item['cantidad'] ?? 0) as num).toDouble(),
    );
      ...
      child: Column(
        ...
        children: [
          ...
          if (ramos.isEmpty)
            ...
            ...ramos.map((item) {
              final String ramoName = item['ramo'] ?? 'Otros';
              final double cantidad = ((item['cantidad'] ?? 0) as num).toDouble();
              final double percent = totalCantidad > 0 ? (cantidad / totalCantidad) : 0.0;
              final String percentStr = '${(percent * 100).toStringAsFixed(0)}%';
              
              Color progressColor;
              switch (ramoName.toLowerCase()) {
                case 'automotor':
                ...
                case 'hogar':
                  ...
                  break;
                case 'vida':
                  progressColor = const Color(0xFF4648D4);
                  break;
                default:
              ...
              return Padding(
                ...
                child: _buildProgressRow(context, ramoName, percentStr, percent, progressColor),
              );
            }).toList(),
        ],
      ),
          ...
          if (_renewals.isEmpty)
            const Center(
              child: Padding(
                ...
                child: Text('Sin renovaciones pendientes esta semana'),
              ),
            )
          else
            ..._renewals.take(5).map((item) {
              final int dias = ((item['dias_restantes'] ?? 30) as num).toInt();
              final String nroPoliza = item['nro_poliza'] ?? '';
              final String cliente = item['cliente_nombre'] ?? 'Cliente';
              final bool isUrgent = dias <= 15;

              return Padding(
                padding: const EdgeInsets.only(bottom: 12.0),
                child: _buildRenewalTile(
                  context,
                  '$dias d',
                  'Póliza $nroPoliza',
                  'Cliente: $cliente',
  ...
  Widget _buildRenewalTile(BuildContext context, String label, String title, String subtitle, {bool isUrgent = false}) {
    final theme = Theme.of(context);
    return Container(
